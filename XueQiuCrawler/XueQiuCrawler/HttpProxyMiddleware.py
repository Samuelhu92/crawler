...
class HttpProxyMiddleware(object):
    ...
    def set_proxy(self, request):
        ...
        if isinstance(request,SplashRequest):
            if proxy["proxy"]:
                request.meta['splash']['args']["proxy"] = proxy["proxy"]
            elif "proxy" in request.meta['splash']['args'].keys():
    ...
    def process_response(self, request, response, spider):
        ...
        """
        if isinstance(request,SplashRequest):
            if "proxy" in request.meta['splash']['args'].keys():
                logger.debug("%s %s %s" % (request.meta['splash']['args']["proxy"], response.status, request.url))
            else:
        ...
        if response.status != 200 \
            ...
            logger.info("%d response status not in spider.website_possible_httpstatus_list,url %s"% (response.status,response.url))
            self.invalid_proxy(request.meta["proxy_index"])
